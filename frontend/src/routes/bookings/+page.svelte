<script>
  ...
  const canceledBookings = $derived(bookings.filter(b => b.status === 'CANCELED'));
...
{#if form?.success}
  <div class="alert alert-success">Booking canceled successfully.</div>
{/if}
...
{:else}
      ...
      {#each pendingBookings as booking}
        ...
        <div class="col-md-6 col-lg-4 mb-3">
          <div class="card h-100">
            <div class="card-body" style="cursor: pointer;" onclick={() => window.location.href = `/rides/${booking.rideId}`}>
              {#if ride}
              ...
              {:else}
                <h6 class="card-title text-muted">Ride details unavailable</h6>
                <p class="card-text"><small class="text-muted">Ride ID: {booking.rideId}</small></p>
              {/if}
            </div>
            <div class="card-footer d-flex justify-content-between align-items-center">
              <div>
                <span class="badge {getStatusClass(booking.status)}">{booking.status}</span>
                <small class="text-muted ms-2">Waiting for approval</small>
              </div>
              <form method="POST" action="?/cancelBooking" use:enhance={handleSubmit}>
  ...
  {#if canceledBookings.length > 0}
    <h5 class="mt-4">
      <span class="badge bg-secondary me-2">{canceledBookings.length}</span>
      Canceled
    </h5>
    <div class="row">
      {#each canceledBookings as booking}
        {@const ride = getRide(booking.rideId)}
        <div class="col-md-6 col-lg-4 mb-3">
          <div class="card h-100">
            <div class="card-body">
